	...
	extends JDialog
{
    ...
    public CapituraDigital(Frame owner) {
        super (owner, true);
        setTitle("Registro de Impressões Digitais");
		...
		prompt.setBorder(
				BorderFactory.createCompoundBorder(
					BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0), "Painel:"),
	...
	protected void init()
	{
		capturer.addDataListener(new DPFPDataAdapter() {
			@Override public void dataAcquired(final DPFPDataEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    makeReport("Modelo de impressão digital capturado.");
                                    setPrompt("Repita o mapeamento da impressão digital.");
                                    process(e.getSample());
                                });
			}
		});
		capturer.addReaderStatusListener(new DPFPReaderStatusAdapter() {
			@Override public void readerConnected(final DPFPReaderStatusEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    makeReport("Leitor de Impressão Digital Conectado.");
                                });
			}
			@Override public void readerDisconnected(final DPFPReaderStatusEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    makeReport("O leitor de impressão digital, está disconectado.");
                                });
			}
		});
		capturer.addSensorListener(new DPFPSensorAdapter() {
			@Override public void fingerTouched(final DPFPSensorEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    makeReport("O leitor capturou seus dados.");
                                });
			}
			@Override public void fingerGone(final DPFPSensorEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    makeReport("O dedo foi removido do leitor de impressão digital.");
                                });
			}
		});
			...
			@Override public void onImageQuality(final DPFPImageQualityEvent e) {
				SwingUtilities.invokeLater(() -> {
                                    if (e.getFeedback().equals(DPFPCaptureFeedback.CAPTURE_FEEDBACK_GOOD))
                                        makeReport("A qualidade do leitura está boa.");
                                    else
                                        makeReport("A qualidade da leitura está ruim");
                                });
			}
		});
	}
	...
	protected void start()
	{
		...
		setPrompt("Use o leitor de impressão digital, para scanear sua digital.");
	}
